...
from ..utils import softwarePaths
...
def joinPalettesWithSameFileNames(paletteFileNames):
	...
	for p in paletteFileNames:
		pkgn=os.path.splitext(os.path.basename(p))[0]
		res[pkgn].append(p)
	return res

def getPallettesPaths():
	for dirPath in softwarePaths:
		dirPath=os.path.join(dirPath, "palettes")
		if not os.path.isdir(dirPath):
			continue
		yield from glob(os.path.join(dirPath, "*.pal"))
